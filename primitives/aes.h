#ifndef AES_H
#define AES_H

#include <cstdint>
#include "util.h"

// message must be a block of 128 bits (utf-8), key must be 256-bit (hex)
std::string aes256_cipher(std::string key, std::string message);

std::string aes256_inv_cipher(std::string key, std::string ciphertext);

// polynomial mutliplication in GF(2^8)
// we reduce modulo x^8 + x^4 + x^3 + x + 1
uint8_t mult_gf(uint8_t a, uint8_t b);

void sub_bytes(uint8_t state[4][4]);

void shift_rows(uint8_t state[4][4]);

void mix_columns(uint8_t state[4][4]);

void add_round_key();


uint8_t mult_gf(uint8_t a, uint8_t b) {
    uint8_t b_copy = b; 
    int b_pos = 1;
    uint8_t res = 0;

    // treat b as sum of powers of 2
    // distribute a onto each element of b
    for (int i = 0; i < 8; i++) { 
        if (b_copy & 1) {
            uint8_t cur_product = a;
            for (int j = 1; j < b_pos; j++) {
                // xTimes
                cur_product = (cur_product >> 7) ? ((cur_product << 1) ^ 0x1b) : (cur_product << 1); 
            }
            res ^= cur_product;
        }   
        b_copy >>= 1;
        b_pos++;
    }

    return res;
}

void sub_bytes(int state[4][4]) {
    static uint8_t swap[256] = {
        
    };
    
    for (int i = 0; i < 4; i++) {
        for (int j =0; j < 4; j++) {
            state[i][j] = swap[state[i][j]];
        }
    }
}

void shift_rows(int state[4][4]) {
    uint8_t temp1, temp2;
    temp1 = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = temp1;

    temp1 = state[2][0];
    temp2 = state[2][1];
    state[2][0] = state[2][2];
    state[2][1] = state[2][3];
    state[2][2] = temp1;
    state[2][3] = temp2;

    temp1 = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = state[3][0];
    state[3][0] = temp1;
}

void mix_columns(uint8_t state[4][4]) {
    for (int j = 0; j < 4; j++) {
        state[0][j] = mult_gf(0x02, state[0][j]) ^ mult_gf(0x03, state[1][j]) ^ state[2][j] ^ state[3][j];
        state[1][j] = state[0][j] ^ mult_gf(0x02, state[1][j]) ^ mult_gf(0x03, state[2][j]) ^ state[3][j];
        state[1][j] = state[0][j] ^ state[1][j] ^ mult_gf(0x02, state[2][j]) ^ mult_gf(0x03, state[3][j]);
        state[1][j] = mult_gf(0x02, state[0][j]) ^ state[1][j] ^ state[2][j] ^ mult_gf(0x02, state[3][j]); 
    }
}

std::string aes256_cipher(std::string key, std::string message) {
    uint8_t state[4][4];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] = static_cast<uint8_t>(message[i + 4*j]);
        }
    }
    
    return ""; 
}

#endif
