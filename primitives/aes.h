...
// message must be 128 bit hex-string
...
// ciphertext is a 128 bit hex-string 
// key must be a hex-string of length 128, 192, or 256 bits
// returns the decrypted plaintext as a hex-string
std::string aes_cipher_inv(std::string key, std::string ciphertext);

// message must be a hex-string
// iv must be 16 byte hex-string, either generated by a cryptographically secure rng, or guaranteed to be distinct for each ciphertext block
...
// iv must be the same 16 byte hex-string used to encrypt the ciphertext
// returns the decrypted plaintext as a hex-string
...
uint32_t* key_expansion(std::string key) {
    std::vector<uint8_t> key_vec = vectorize_hex(key);
    int Nk = key_vec.size() / 4; 
    int num_rounds;   
    switch(Nk) {
        case 4:
            num_rounds = 10;
        ...
        case 8:
            ...
            break;
        default:
            std::cerr << "invalid key length of: " << Nk << " words\n";
            exit(1);
    }
    ...
    for (int i = 0; i < Nk; i++) { 
        round_keys[i] = ( 
            (static_cast<uint32_t>(key_vec[4*i]) << 24) | 
            (static_cast<uint32_t>(key_vec[4*i+1]) << 16) | 
            (static_cast<uint32_t>(key_vec[4*i+2]) << 8) |
            (static_cast<uint32_t>(key_vec[4*i+3]))
        );
    }
...
std::string aes_cipher(std::string key, std::string message) {
    ...
    switch(key.size() * 4) {
        ...
        case 256:
            ...
            break;
        default:
            std::cerr << "invalid key length of: " << key.size()*4 << " bits\n";
            exit(1);
    }
    ...
    std::stringstream state_hex; 
    state_hex << std::hex << std::setfill('0');
    for (int j = 0; j < 4; j++) {
        for (int i = 0; i < 4; i++) {
            state_hex << std::setw(2) << static_cast<unsigned int>(state[i][j]);
        }
    }

    return state_hex.str();
}
...
std::string aes_cipher_inv(std::string key, std::string ciphertext) {       
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            state[i][j] = ciphertext_vec[i + 4*j];
        }
    }
    // print_state(state);
    uint32_t* round_keys = key_expansion(key);
...
std::string counter_mode_encrypt(std::string key, std::string message, std::string counter) {
    ...
    ciphertext << std::hex << std::setfill('0');
    ...
    for (int i = n-(n % 16); i < n; i++) { 
        ciphertext << std::setw(2) << static_cast<unsigned int>(pad[i-(n-(n % 16))] ^ message_vec[i]);
    }
...
std::string counter_mode_decrypt(std::string key, std::string ciphertext, std::string counter) { 
    ...
    plaintext << std::hex << std::setfill('0');
    std::vector<uint8_t> pad; 
    ...
    pad = vectorize_hex(aes_cipher(key, counter)); 
    for (int i = n-(n % 16); i < n; i++) {
        plaintext << std::setw(2) << static_cast<unsigned int>(pad[i-(n-(n % 16))] ^ ciphertext_vec[i]);
    }
